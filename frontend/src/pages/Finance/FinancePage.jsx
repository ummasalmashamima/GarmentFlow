...
function FinancePage() {
  ...
  return <div className="master-data-page finance-page">
    ...
    {modal === 'invoice-form' && <div className="modal-backdrop" role="presentation"><div className="modal-card order-modal-card" role="dialog" aria-modal="true" aria-labelledby="invoice-form-title"><div className="modal-header"><div><p className="eyebrow">Accounts receivable</p><h2 id="invoice-form-title">Create Invoice</h2></div><button aria-label="Close invoice form" className="icon-button" onClick={closeModal} type="button">×</button></div><form className="master-data-form" onSubmit={submitInvoice}><div className="inline-preview">Invoice totals are calculated on the backend. Confirmed, in-progress, and delivered Sales Orders are available for invoicing.</div><div className="form-grid"><label className="form-field full-width"><span>Eligible Sales Order *</span><select onChange={(event) => chooseOrder(event.target.value)} required value={invoiceForm.sales_order_id}><option value="">{eligibleLoading ? 'Loading eligible orders…' : 'Select Sales Order'}</option>{eligibleOrders.map((order) => <option key={order.id} value={order.id}>{order.sales_order_number} · {partyLabel(order)} · {statusLabel(order.status)} (qty: {formatNumber(order.delivered_quantity > 0 ? order.delivered_quantity : (order.confirmed_quantity || order.ordered_quantity))})</option>)}</select></label><label className="form-field"><span>Invoice date *</span><input onChange={(event) => setInvoiceForm((current) => ({ ...current, invoice_date: event.target.value }))} required type="date" value={invoiceForm.invoice_date} /></label><label className="form-field"><span>Due date *</span><input onChange={(event) => setInvoiceForm((current) => ({ ...current, due_date: event.target.value }))} required type="date" value={invoiceForm.due_date} /></label><label className="form-field full-width"><span>Remarks</span><textarea onChange={(event) => setInvoiceForm((current) => ({ ...current, remarks: event.target.value }))} rows="2" value={invoiceForm.remarks} /></label></div>{invoiceSource && <section className="order-detail-section"><div className="order-section-heading"><div><p className="eyebrow">Order item lines</p><h3>{invoiceSource.sales_order_number}</h3></div><span className={statusClass(invoiceSource.status)}>{statusLabel(invoiceSource.status)}</span></div><div className="table-wrap"><table className="master-data-table"><thead><tr><th>Product</th><th>Variant</th><th>Quantity</th><th>Unit</th><th>Unit price</th></tr></thead><tbody>{(invoiceSource.items || []).map((item) => <tr key={item.id}><td>{item.product?.name || '—'}</td><td>{item.product_variant?.sku || 'Product level'}</td><td>{formatNumber(item.quantity ?? (item.delivered_quantity > 0 ? item.delivered_quantity : (item.confirmed_quantity || item.ordered_quantity)))}</td><td>{item.unit?.code || item.unit?.symbol || '—'}</td><td>{formatMoney(item.unit_price)}</td></tr>)}</tbody></table></div></section>}<div className="modal-actions"><button className="secondary-button" onClick={closeModal} type="button">Cancel</button><button className="primary-button" disabled={busy || !invoiceSource} type="submit">{busy ? 'Creating…' : 'Create draft invoice'}</button></div></form></div></div>}
